...
(function() {
  ...
  var LeadBoxes = function() {
    ...
    window.LEADBOX.badge = function(site_id, app_type, app_id, link_id, demo, cb) {
      var url = popupdelivery_api + '/api/preview?preview=1&site_id=' + site_id + (tinyLeadGetParameterByName('app_id') ? '&app_id=' + tinyLeadGetParameterByName('app_id') : '');
...
(function() {
  ...
  if (document.currentScript) {
    ...
    safetyUrl += '?preview=1' + (tinyLeadGetParameterByNameWindow('app_id') ? '&app_id=' + tinyLeadGetParameterByNameWindow('app_id') : '') + '&cookie_id=' + util.get('pd_pixel') + '&site_id=' + site_id + '&referer=' + encodeURIComponent(document.referrer) + '&is_mobile=' + (is_mobile ? 1 : 0);
  }
