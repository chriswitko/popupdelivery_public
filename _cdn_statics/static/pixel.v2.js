...
(function() {
  ...
  var LeadBoxes = function() {
    ...
    this.resize = function() {
      tinyLeadGetSize(function(documentWidth, documentHeight, width, height) {
        if(leadboxes.popup) {
          ...
          if(width < parseInt(leadboxes.popup.style.origWidth)) {
            leadboxes.popup.style.minWidth = (width - 50) + 'px';
            leadboxes.popup.style.width = leadboxes.popup.style.minWidth;
          } else {
            leadboxes.popup.style.width = parseInt(leadboxes.popup.style.origWidth) + 'px';//'600px';
            leadboxes.popup.style.minWidth = leadboxes.popup.style.width;
          }
          leadboxes.popup.style.height = parseInt(leadboxes.popup.style.height) + "px";
          leadboxes.popup.style.minHeight = parseInt(leadboxes.popup.style.height) + "px";
    ...
    var isPopupResized = false;
    var itsFirstTime = false;
    ...
    var onMessageHandler = function(event) {
      ...
      if (event.data && event.data.type == "resize_popup") {
        tinyLeadGetSize(function(documentWidth, documentHeight, width, height) {
          if(event.data.value.action !== 'resize') {
          ...
          leadboxes.popup.style.left = '50%';
          leadboxes.popup.style.marginLeft = -((parseInt(leadboxes.popup.style.width)) / 2) + "px";
          // if(event.data.value.has_hero && event.data.value.action === 'image_loaded') {
            leadboxes.popup.style.display = 'block';
          // } else if(!event.data.value.has_hero) {
            // leadboxes.popup.style.display = 'block';
          // }
        });
      }
    ...
    window.LEADBOX.popup = function(site_id, app_type, app_id, link_id, demo, cb) {
      ...
      var url = 'https://s3.amazonaws.com/popupdelivery/apps/' + app_id + '.html?cookie_id=' + util.get('pd_pixel') + '&site_id=' + site_id;
  ...
  var load = function() {
    if (!window.ae417310535411e38f960800200c9a66) {
      ...
      window.loadAddPopUpDeliveryStuff();
    }
...
(function() {
  window.getPopUpDeliveryCurrentScript = document.currentScript;
  window.loadAddPopUpDeliveryStuff = function() {
    var util = new window.CookieUtil();
    ...
    if (window.getPopUpDeliveryCurrentScript) {
      safetyUrl = getSegment(window.getPopUpDeliveryCurrentScript.src);
      var site_id = window.getPopUpDeliveryCurrentScript.getAttribute('site_id');
      ...
      if (lang.indexOf('_') !== -1)
          lang = lang.split('_')[0];      
      safetyUrl += '?cookie_id=' + util.get('pd_pixel') + '&site_id=' + site_id + '&referer=' + encodeURIComponent(document.referrer) + '&is_mobile=' + (is_mobile ? 1 : 0) + '&language=' + lang;
    }
