...
gulp.task('client:inject', function() {
  return gulp.src('client/index.html')
    .pipe(inject(gulp.src(['client/**/*.js', '.tmp/*.tpl.js']).pipe(angularFilesort()), {
      ignorePath: ['client', '.tmp', 'assets/js/demo.js', 'assets/js/preview.js', 'assets/internal_testing/pixel.dev.js'],
      addRootSlash: false
    }))
    .pipe(gulp.dest('.tmp/'));
});
...
gulp.task('client:watch', function() {
  ...
  gulp.watch('client/styles/**/*.scss', gulp.series(['sass'], reload));
...
gulp.task('client:copy-assets-dist', gulp.parallel([
  function copyBootstrapFonts() {
...
gulp.task('client:default', gulp.series(['client:lint-dev', 'client:template', 'client:inject', 'sass', 'client:copy-i18n', 'client:serve', 'client:watch']));
gulp.task('client:dist', gulp.series(['client:lint', 'client:template', 'client:inject', 'sass', 'client:copy-i18n-dist', 'client:copy-fonts-dist', 'client:copy-assets-dist', 'client:build']));
gulp.task('client:verify', gulp.series(['client:lint', 'client:template', 'client:inject', 'sass', 'client:copy-i18n-dist', 'client:copy-fonts-dist', 'client:copy-assets-dist']));
