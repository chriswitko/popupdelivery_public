...
function GetResponse(apiKey,apiUrl, print_errors) {
    var apiURL = apiUrl || 'https://api.getresponse.com/v3';
    ...
    var send = function (data, cb) {
      console.log('data', data.params);
        ...
        request.post(apiURL + '/' + data.method, options, function (error, response, body) {
    ...
    this.prepRequest = function (method, params) {
        var data = params;
    ...
    this.addContact = function ($campaign, $name, $email, $action, $cycle_day, $customs, cb) {
        ...
        var request = $this.prepRequest('contacts', params);
        send(request, cb);
    };
